#!/usr/bin/env python3
"""
Универсальный скрипт для применения улучшенного форматирования ко всем Notion страницам
"""
...
def apply_formatting_to_event(event_id, config, headers):
    """Применяет форматирование к конкретному событию"""
    
    print(f"🎨 Обработка события: {event_id}")
    ...
    if not event_data:
        print(f"   ❌ Событие не найдено в БД")
        conn.close()
        return False
    ...
    if not notion_result:
        print(f"   ❌ Notion страница не найдена")
        conn.close()
        return False
    ...
    if not clear_page_content(page_id, headers):
        print(f"   ❌ Ошибка очистки страницы")
        return False
    
    # Добавляем блоки по частям
    chunk_size = 50
    success = True
    ...
    if success:
        print(f"   ✅ Форматирование применено ({len(blocks)} блоков)")
...
def main():
    ...
    cursor.execute('''
        SELECT pe.event_id, pe.event_title, nss.page_id
    ...
    print(f"🎯 Применение улучшенного форматирования к {len(events)} событиям")
    print("=" * 70)
    ...
    for event_id, event_title, page_id in events:
        try:
            success = apply_formatting_to_event(event_id, config, headers)
            if success:
                success_count += 1
            else:
                error_count += 1
