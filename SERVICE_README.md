...
- **Разработка**: LOG_LEVEL=DEBUG, LOG_RETENTION_DAYS=7, LOG_MAX_SIZE_MB=10
- **Продакшн**: LOG_LEVEL=INFO, LOG_RETENTION_DAYS=90, LOG_MAX_SIZE_MB=100
...
### 🆕 Новые возможности
### 🛠️ Инструменты управления логами

- **`log_manager.py`** - комплексное управление логами с анализом и оптимизацией
- **`cron_log_cleanup.py`** - автоматическая очистка логов для cron задач
- **Ротация логов** - автоматическое создание резервных копий при превышении размера
- **Очистка по времени** - автоматическое удаление старых логов
- **Консолидация** - устранение дублирующих файлов
