                ...
                for video_file in video_files:
                    if 'compressed' not in video_file.name.lower():
                        ...
                        if video_processed:
                            ...
                            # Добавляем этап video_compression, если его еще нет
                            existing_steps = status.status_data['files'][video_file.name].get('processing_steps', [])
                            if not any(step.get('step') == 'video_compression' for step in existing_steps):
