...
- **🗄️ SQLite State Management** - полное отслеживание состояния системы
- **🎨 Structured Notion Pages** - профессиональное форматирование страниц
- **🔧 59 Utility Tools** - комплексное управление системой
- **🚫 Event Exclusion System** - гибкая фильтрация событий
- **📊 Database Viewer** - интерактивный просмотр состояния
- **🔄 Content Synchronization** - автоматическая синхронизация контента
...
├── src/
│   ├── service_manager.py        # Основной сервис-оркестратор
│   ├── config_manager.py         # Управление конфигурацией
│   ├── event_exclusions.py       # Система фильтрации событий
│   ├── notion_templates.py       # Шаблоны страниц Notion
│   ├── telegram_api.py           # Уведомления Telegram
│   ├── prompt_manager.py         # Управление AI промтами
│   └── handlers/                 # Модульные обработчики
│       ├── base_handler.py       # Базовый класс обработчика
│       ├── account_handler.py    # Управление аккаунтами
│       ├── calendar_handler.py   # Обработка календарей
│       ├── calendar_integration_handler.py  # Интеграция календарей
│       ├── media_handler.py      # Обработка медиа файлов
│       ├── transcription_handler.py  # Транскрипция аудио
│       ├── summary_handler.py    # Саммаризация контента
│       ├── notion_handler.py     # Интеграция с Notion
│       ├── notion_api.py         # API клиент Notion
│       ├── state_manager.py      # Управление состоянием SQLite
│       ├── smart_report_generator.py  # Умная генерация отчетов
│       └── metrics_handler.py    # Метрики производительности
├── tools/                        # Утилиты (59 инструментов)
│   ├── db/                       # Просмотр базы данных
│   ├── db_interactive.py         # Интерактивный просмотр БД
│   ├── db_viewer.py              # Командный просмотр БД
│   ├── improve_notion_formatting.py  # Форматирование Notion
│   ├── check_transcription_sync_status.py  # Проверка синхронизации
│   ├── generate_sync_report.py   # Комплексные отчеты
│   ├── enhance_notion_pages.py   # Улучшение страниц Notion
│   └── [50+ других утилит]
├── data/
│   ├── system_state.db           # SQLite база данных (основное состояние)
│   ├── service_cache.json        # Кэш сервиса
│   ├── service_state.json        # Состояние сервиса
│   ├── performance_stats.json    # Метрики производительности
│   └── [различные директории данных]
...
## 🆕 Последние изменения (v3.0)

### 🗄️ SQLite State Management
- **Persistent Database** - `system_state.db` с полным отслеживанием состояния
- **Event Processing** - управление полным жизненным циклом событий
- **Media Tracking** - статус обработки файлов и метаданные
- **Transcription Management** - отслеживание обработки аудио-в-текст
- **Summary Tracking** - управление AI-генерируемым контентом
- **Notion Sync Status** - создание страниц и синхронизация контента
- **Folder Creation** - управление папками встреч

### 🎨 Advanced Notion Integration
- **Structured Page Formatting** - профессиональные макеты страниц с секциями
- **Content Synchronization** - автоматическая синхронизация транскрипций, саммари и анализа
- **Property Management** - полные метаданные событий в свойствах Notion
- **Drive Folder Links** - прямые ссылки на папки встреч
- **Event ID Tracking** - уникальные идентификаторы для всех событий
- **Content Blocks** - форматирование с заголовками, callout'ами и списками

### 🔧 Comprehensive Tool Suite
- **Database Viewer** - интерактивный и командный просмотр базы данных
- **Sync Status Checker** - мониторинг синхронизации в реальном времени
- **Content Enhancement** - автоматическое улучшение страниц Notion
- **Formatting Tools** - создание профессиональной структуры страниц
- **Report Generation** - комплексные отчеты о состоянии системы
- **Data Migration** - инструменты для консистентности и очистки данных

### 🚫 Event Exclusion System
- **Flexible Filtering** - исключение событий по ключевым словам и regex
- **Unified Configuration** - единая настройка в `.env` для всех исключений
- **Real-time Processing** - фильтрация событий во время обработки календаря
- **Configurable Rules** - легкое добавление/удаление паттернов исключений

### 🧠 Умная система уведомлений (v2.2.0)
